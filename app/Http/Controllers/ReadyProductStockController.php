...
class ReadyProductStockController extends Controller
{
    ...
    {
        try {
            // Handle query=all for getting all ready product stock items
            if ($request->query('query') === 'all') {
                $stockItems = ReadyProductStockItem::with([
                    'rawMaterial:id,name',
                    'readyProductStock:id,date,reference'
                ])
                ->where('current_stock_kg', '>', 0)
                ->orderBy('created_at', 'desc')
                ->get();
                
                // Load sale product names from Product table using sale_product_name as ID
                foreach ($stockItems as $item) {
                    if ($item->sale_product_name) {
                        $product = \App\Models\Product::find($item->sale_product_name);
                        if ($product) {
                            $item->sale_product_display_name = $product->name;
                        }
                    }
                }
                ...
                return response()->json([
                    'success' => true,
                    'data' => $stockItems,
                    'message' => 'Ready product stock items retrieved successfully'
                ]);
            }
    ...
    public function renewStock($id)
    {
        ...
        try {
            ...
            // Smart renewal: Add remaining kg to current stock for each item
            foreach ($stock->items as $item) {
                // Current values
                $currentStockKg = $item->current_stock_kg;
                $remainingKg = $item->remaining_kg;
                $bagsWeightKg = $item->bags_weight_kg > 0 ? $item->bags_weight_kg : 10; // Default 10kg per bag
                
                // Calculate new total stock = current stock + remaining kg
                $newTotalStockKg = $currentStockKg + $remainingKg;
                
                // Calculate how many complete bags can be made from new total
                $newStockBags = floor($newTotalStockKg / $bagsWeightKg);
                
                // Calculate new remaining kg after making bags
                $newRemainingKg = $newTotalStockKg - ($newStockBags * $bagsWeightKg);
                
                // Update the item with new calculated values
                $item->update([
                    'current_stock_kg' => $newTotalStockKg,
                    'current_stock_bags' => $newStockBags,
                    'remaining_kg' => $newRemainingKg
                ]);
                
                // Track renewal details for user feedback
                $renewalDetails[] = [
                    'product_name' => $item->ready_product_name,
                    'previous_stock_kg' => $currentStockKg,
                    'added_remaining_kg' => $remainingKg,
                    'new_total_stock_kg' => $newTotalStockKg,
                    'new_bags' => $newStockBags,
                    'new_remaining_kg' => $newRemainingKg
                ];
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Stock renewed successfully! Remaining kg added to current stock.',
                'renewal_details' => $renewalDetails,
                'data' => $stock->load('items')
            ]);
        } catch (\Exception $e) {
            DB::rollback();
            return response()->json([
